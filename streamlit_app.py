...
with tab1:
    ...
    fig.update_layout(scene=dict(xaxis_title='Queda (m)', yaxis_title='Vazão (m³/s)', zaxis_title='Eficiência (%)'),
...
# --- ABA 3: DESPACHO ---
with tab3:
    st.subheader("Simulação de Despacho Ótimo (Eq. 7)")
    c1, c2 = st.columns(2)
...
# --- ABA 4: MÉDIAS ---
with tab4:
    st.subheader("Cálculo de Parâmetros Médios (Séries Temporais)")
    ...
    if up:
        ...
        if {'Queda', 'Vazao'}.issubset(df.columns):
            ...
            st.line_chart(df['Rend_Otimo'])
        else: st.error("CSV inválido.")

# --- ABA 5: FÓRMULAS COMPLETAS ---
with tab5:
    ...
    with col_eq1:
        ...
        st.latex(r'''
            \eta_{t}(h,q) = a_{02}q^{2} + a_{20}h^{2} + a_{11}qh + a_{01}q + a_{10}h + a_{00}
        ''')
        st.subheader("2. Problema de Otimização (Eq. 6)")
        st.latex(r'''
            \text{Max } P_{tot} = \sum_{i=1}^{n} (\eta_t(h, q_i) \cdot q_i \cdot h \cdot \rho \cdot g)
        ''')

    with col_eq2:
        st.subheader("3. Perda Hidráulica (Eq. 1 e 15)")
        st.latex(r'''
            \Delta h_{med} = \frac{\sum (E_m \cdot \Delta h_m)}{\sum E_m}
        ''')
        st.subheader("4. Rendimento Médio (Eq. 2)")
        st.latex(r'''
            \eta_{med} = \frac{\sum_{m=1}^{N} (E_m \cdot \eta_{conj,m})}{\sum_{m=1}^{N} E_m}
        ''')
